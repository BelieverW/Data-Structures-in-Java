...
public class RunLengthEncoding implements Iterable {
  ...
  public RunLengthEncoding(int width, int height, int[] red, int[] green,
    ...
    for(int counter = 0; counter < runLengths.length; counter++){
      DListNode tempList = new DListNode((short)red[counter], (short)green[counter],
                                         (short)blue[counter], runLengths[counter]);
      this.list.insertEnd(tempList);
    }
  }
  ...
  public RunIterator iterator() {
    // Replace the following line with your solution.
    RunIterator myIterator = new RunIterator(list);
    return myIterator;
  ...
  public PixImage toPixImage() {
    ...
    for(RunIterator i = iterator(); i.hasNext();){
      ...
      while(array[0] > 0){
        if( column < width){
          //System.out.println("" + width+","+ height+","+row+","+column);
          newPixImage.p[row][column][0] = (short)array[1];
          newPixImage.p[row][column][1] = (short)array[2];
          newPixImage.p[row][column][2] = (short)array[3];
          column++;
        }else{
          row++;
          column = 0;
  ...
  public String toString() {
    // Replace the following line with your solution.
    String result = "[";
    DListNode current = list.getHead();
    while(current != null){
  ...
  public RunLengthEncoding(PixImage image) {
    ...
    // at the end.
    //DListNode tempList1 = new DListNode();
    DListNode tempList = new DListNode();
    int counter = 1;
    list = new DList();
    ...
    for(int j = 0; j < height; j++){
      for(int i = 0; i < width; i++){
        tempList = new DListNode(image.p[j][i][0], image.p[j][i][1], image.p[j][i][2]);
        if(list.getHead() == null){
          tempList.setLength(counter);
          list.insertEnd(tempList);
        }else if(list.getTail().equals(tempList)){
          list.getTail().addLength();
        }else{
          tempList.setLength(counter);
          list.insertEnd(tempList);
        }
  ...
  public boolean equal(int[] self, int[] other){
  ...
  public void check() {
    // Your solution here.
    RunIterator iterator = new RunIterator(list);
    int sum = 0;
    ...
    if(sum != (width * height)){
      System.out.println("****Wrong number of pixels: "+ (width * height) + ", " + sum);
    }
  }
  ...
  public void setPixel(int x, int y, short red, short green, short blue) {
    // Your solution here, but you should probably leave the following line
    //   at the end.
    DListNode setNode = new DListNode(red, green, blue);
    DListNode lastNode = list.getHead();
    DListNode currentNode = list.getHead();
