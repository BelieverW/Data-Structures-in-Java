...
class SibTreeNode extends TreeNode {
  ...
  public void insertChild(Object item, int c) throws InvalidNodeException {
    ...
    TreeNode node = new SibTreeNode(myTree, item);
    TreeNode nextSibling, prevSibling;
    ...
    if (!isValidNode()){
      ...
      if (children() == 0) {
        ((SibTreeNode) node).parent = this;
        ((SibTreeNode) this).firstChild = (SibTreeNode) node;
        myTree.size++;
      } else {
        // this node has more than 1 child.

        // get the child next to insert node aftering this execution.
        nextSibling = child(c);
        if (!nextSibling.isValidNode()) {
          if (c < 1) {
            nextSibling = firstChild;
            ((SibTreeNode) node).nextSibling = (SibTreeNode) nextSibling;
            ((SibTreeNode) node).parent = this;
            firstChild = (SibTreeNode) node;
          } else {
            nextSibling = child(children());
            ((SibTreeNode) nextSibling).nextSibling = (SibTreeNode) node;
            ((SibTreeNode) node).parent = this;
          }
          myTree.size++;
        } else {
          if (c == 1) {
            ...
            prevSibling = child(c - 1);
            ((SibTreeNode) node).parent = this;
            ((SibTreeNode) node).nextSibling = (SibTreeNode) nextSibling;
            ((SibTreeNode) prevSibling).nextSibling = (SibTreeNode) node;
          }
          myTree.size++;
        }
      }
  ...
  public void removeLeaf() throws InvalidNodeException {
    ...
    // method 1 without add another method.
    /*if (!isValidNode()){
      throw new InvalidNodeException();
    } else {
      ...
      if (children() == 0) {
        ...
        if (parentNode == null) {
          ...
          SibTreeNode prevNode, node;
          prevNode = ((SibTreeNode) parentNode).firstChild;
          node = prevNode.nextSibling;
          if (prevNode == this){
            ((SibTreeNode) parentNode).firstChild = this.nextSibling;
          } else {
            while (prevNode != null) {
              if (node != null) {
                if (node == this){
    ...
    // method 2, add another method numChild defined below.
    if (!isValidNode()) {
      throw new InvalidNodeException();
    } else {
      if (children() == 0) {
        ...
        if (parentNode == null) {
          ...
          int index = numChild(parentNode, this);
          if (index == 1){
            ((SibTreeNode) parentNode).firstChild = this.nextSibling;
          } else {
            TreeNode prevNode = ((SibTreeNode) parentNode).child(index - 1);
            ((SibTreeNode) prevNode).nextSibling = this.nextSibling;
          }
          this.valid = false;
          myTree.size--;
        }
  ...
  public int numChild(TreeNode parent, SibTreeNode child){
