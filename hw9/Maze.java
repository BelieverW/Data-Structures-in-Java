...
public class Maze {
  ...
  public Maze(int horizontalSize, int verticalSize) {
    ...
    DisjointSets mazeCell = new DisjointSets(horiz * vert);
    WallArray[] walls = new WallArray[(horiz * (vert - 1)) + ((horiz - 1) * vert)];
    int counter = 0;
    WallArray temp;

    for (j = 0; j < vert - 1; j++) {
      for (i = 0; i < horiz; i++) {
        walls[counter++] = new WallArray(i, j, false);
      }
    }
    for (i = 0; i < horiz - 1; i++) {
      for (j = 0; j < vert; j++) {
        walls[counter++] = new WallArray(i, j, true);
      }
    }
    ...
    while (counter != 0) {
      ...
      temp = walls[randIndex];
      walls[randIndex] = walls[counter - 1];
      walls[counter - 1] = temp;
      counter--;
    }
    while (counter < walls.length) {
      ...
      if (temp.isVwalls()) {
        ...
        index1 = i * vert + j;
        index2 = (i + 1) * vert + j;
        if (mazeCell.find(index1) == mazeCell.find(index2)) {
        } else {
          mazeCell.union(mazeCell.find(index1), mazeCell.find(index2));
          vWalls[i][j] = false;
        }
      } else {
        int index1, index2;
